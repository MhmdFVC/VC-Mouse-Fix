...
/*
void Patch2(int address, void* data, int size)
{
	unsigned long protect[2];
...
void Unprotect(int address, int size)
{
	unsigned long protect[2];
	VirtualProtect((void *)address, size, PAGE_EXECUTE_READWRITE, &protect[0]);
}
void Nop(int address, int size)
{
	unsigned long protect[2];
	VirtualProtect((void *)address, size, PAGE_EXECUTE_READWRITE, &protect[0]);
...
void RedirectCall(int address, void *func)
{
	int temp = 0xE8;
...
void RedirectJump(int address, void *func)
{
	int temp = 0xE9;
	Patch((void *)address, &temp, 1);
	temp = (int)func - ((int)address + 5);
...
/*
void SetPointer(int address, void *value)
{
	Patch((void *)address, &value, 4);
}

void AdjustPointer(int address, void *value, int offset)
{
	int result;
